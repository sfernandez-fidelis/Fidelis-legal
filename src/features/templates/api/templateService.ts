  ...
  async createTemplate(organizationId: string, userId: string, type: ContractType) {
    ...
    const { data: createdVersion, error: versionError } = await supabase
      ...
      .insert({
        ...
        change_note: 'Borrador inicial desde el sistema predeterminado',
        created_by: userId,
        created_at: now,
      })
      .select('id')
  ...
  async saveDraft(
    ...
    const { data: version, error: versionError } = await supabase
      ...
      .insert({
        ...
        content,
        change_note: changeNote?.trim() || 'Borrador actualizado',
  ...
  async rollbackTemplate(
    ...
    const { data: rollbackVersion, error: versionError } = await supabase
      ...
      .insert({
        ...
        change_note: changeNote?.trim() || `Reversión a la v${target.versionNumber}`,
