...
export const generateCounterGuaranteePrivateHTML = (
  ...
  const policiesStr = data.policies
    .map((policy, index) => {
      ...
      return decorateVariableHtml(
        `Clase ${policyTypeFormatted}, con número de póliza ${policyNumWords}, por un monto afianzado de hasta ${amountWords} Quetzales${exactosStr} (Q.${policy.amount.toLocaleString('es-GT', {
        ...
        <p style="margin-bottom: 0;">
          ...
          ${allParties
            .map((party, index) => {
              ...
              return decorateVariableHtml(text, mode);
            })
            .join(', ')}.
