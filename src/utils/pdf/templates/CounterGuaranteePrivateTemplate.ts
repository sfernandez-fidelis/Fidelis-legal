...
import { formatDPI, formatDateInWords, formatNumberWithWords, formatPolicyType, numberToWords } from '../formatters';
...
export const generateCounterGuaranteePrivateHTML = (data: CounterGuaranteeData) => {
  ...
  const policiesStr = data.policies.map((p, i) => {
    const policyTypeFormatted = formatPolicyType(p.type);
    const policyNumWords = formatNumberWithWords(p.number);
    const amountWords = numberToWords(p.amount);
    const exactosStr = p.amount % 1 === 0 ? ' exactos' : '';
    
    return `Clase ${policyTypeFormatted}, con número de póliza ${policyNumWords}, por un monto afianzado de hasta ${amountWords} Quetzales${exactosStr} (Q.${p.amount.toLocaleString('es-GT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })})` + (i === data.policies.length - 1 ? '. ' : ' y, ');
  }).join('');

  const dateStr = formatDateInWords(data.contractDate);
  const allParties = [data.principal, ...data.guarantors];
  const entityNames = allParties.filter(p => p.isRepresenting).map(p => `"${p.entityName?.toUpperCase()}"`).join(' e ');
      ...
      <div style="margin-top: 60px; border-top: 1px solid #000; padding-top: 20px;">
        <p style="margin-bottom: 0;">
          ...
          ${allParties.map((p, i) => {
            ...
            let text = `${letters[i]}) ${p.name.toUpperCase()}, quien se identifica con el Documento Personal de Identificación -DPI- con Código Único de Identificación -CUI- ${formatDPI(p.cui)} extendido por el Registro Nacional de las Personas de la República de Guatemala`;
            if (p.isRepresenting) {
              text += `, actuando en calidad de ${p.role.toUpperCase()}, de la entidad denominada ${p.entityName?.toUpperCase()}, lo cual acredita con el Acta Notarial de mi nombramiento autorizada en la ciudad de Guatemala por el Notario ${p.notaryName}, el ${formatDateInWords(p.actDate)}, inscrita en el Registro Mercantil General de la República al número de registro ${formatNumberWithWords(p.regNumber || '')}, folio ${formatNumberWithWords(p.regFolio || '')}, del libro ${formatNumberWithWords(p.regBook || '')} de Auxiliares de Comercio`;
            }
            return text;
          }).join(', ')}. 
