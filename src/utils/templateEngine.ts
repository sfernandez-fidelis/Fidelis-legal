...
function buildPartyTemplateString(party: PartyDetails) {
  if (!party.name) {
    return '';
  }
  ...
  return `${base}, quien actúa en representación de <b>${party.entityName}</b>, calidad que acredita con el acta notarial autorizada en esta ciudad por el Notario ${party.notaryName} de fecha ${formatDateInWords(party.actDate, { includeYearLabel: true })}, inscrita en el Registro Mercantil General de la República al número ${formatNumberWithWords(party.regNumber || '')}, folio ${formatNumberWithWords(party.regFolio || '')}, libro ${formatNumberWithWords(party.regBook || '')}`;
}

function buildSignaturesTemplate(signatureNames: string[]) {
  return `
...
function buildAuthenticaTemplate(data: CounterGuaranteeData, dateStr: string, firmasStr: string) {
...
export function compileTemplate(templateStr: string, data: CounterGuaranteeData): string {
  ...
  let compiled = templateStr.replace(/<mark(?:[^>]*)>\{\{([^}]+)\}\}<\/mark>/g, '{{$1}}');
  ...
  const principalStr = buildPartyTemplateString(data.principal) || '[DATOS DEL FIADO]';
  compiled = compiled.replace(/\{\{DATOS_FIADO\}\}/g, principalStr);

  const guarantorsStr =
    data.guarantors && data.guarantors.length > 0
      ? data.guarantors.map((guarantor) => buildPartyTemplateString(guarantor)).join('; y ')
  ...
  const firmasStr = buildSignaturesTemplate(data.signatureNames ?? []);
  compiled = compiled.replace(/\{\{FIRMAS\}\}/g, firmasStr);
  compiled = compiled.replace(/\{\{AUTENTICA\}\}/g, buildAuthenticaTemplate(data, dateStr, firmasStr));
